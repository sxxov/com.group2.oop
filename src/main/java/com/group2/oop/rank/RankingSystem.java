...
public class RankingSystem implements Service {
    ...
    public RankingSystem(Service next) {
        this.next = next;
    }

    private final CarbonCreditAccountRepository carbonCreditAccountRepository = D.get(CarbonCreditAccountRepository.class);

  private final CarbonCreditAccount carbonCreditAccount = D.get(CarbonCreditAccount.class);
    ...
    public void init(Engine engine) {
        ...
        for (int i = 0; i < userIds.size(); i++) {
            ...
            System.out.println((i + 1) + ". User " + userId + " - Received Carbon Credits: " + carbonCredits);
        }
