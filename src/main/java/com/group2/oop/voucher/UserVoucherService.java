...
public class UserVoucherService implements Service {

	public final AccountManager account = D.get(AccountManager.class);
	public final Scanner scanner = D.get(Scanner.class);

	private final AdminVoucherRepository repository = D.get(
		AdminVoucherRepository.class
	);

	private final UserVoucherRepository repositoryUser = D.get(
		UserVoucherRepository.class
	);

	public void addVoucher(UserVoucherRepository voucherUser) {
		...
		while (true) {
			...
			switch (choice) {
				case 1:
					...
					for (AdminVoucherRepository voucher : repository.all()) {
						System.out.println(
							"Code: " +
							voucher.getCode() +
							", Description: " +
				...
				case 3:
					...
					for (UserVoucherRepository voucherUser : repositoryUser.all()) {
						System.out.println(
							"Code: " +
							voucherUser.getCode() +
							", Description: " +
							voucherUser.getDescription()
						);
					}
					break;
				case 4:
			...
			if (voucher.getCode().equals(code)) {
				...
				if (creditsRequired > 0) {
					if (credits >= creditsRequired) {
						...
						System.out.println(
							"Deducted " + creditsRequired + " credits."
						);

						deductCredits(creditsRequired); // Deduct the required credits
						addVoucher(
							new UserVoucherRepository(
								voucher.getCode(),
								voucher.getDescription()
							)
						);
						...
						System.out.println(
							"Total credits left: " + getCredits()
						);

						break;
					} else {
						System.out.println(
							"Insufficient credits to redeem this voucher."
						);
					}
				} else {
